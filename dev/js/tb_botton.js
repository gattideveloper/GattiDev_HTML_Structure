...
class DT_Botton{
	...
	renderRows(){
		...
		for(i = pointer; i < limit; i++){
			...
			values.forEach(cell =>{
				data += `<td class="row_status">${cell}</td>`;
			});
	...
	getDatos(){
		...
		for(let i = 0; i < this.selected.length; i++){
			const {id, values} = this.selected[i];
			const row = values;
			const Primer_Recorte = row[0].indexOf("'");
			const Segundo_Recorte = row[0].lastIndexOf("'");
			const Ubicacion_Final = Primer_Recorte + 1;
			const Resultado = row[0].slice(Ubicacion_Final, Segundo_Recorte);
